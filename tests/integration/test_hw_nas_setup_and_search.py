...
from tests.integration.samples.sample_MLP import sample_MLP
...
class TestHWNasSetupAndSearch:
    ...
    def setUp(self):
        ...
        self.hwnas_cfg = HWNASConfig(config_path="tests/integration/configs/hwnas_config.yaml")
    ...
    def test_generate_for_hw_platform(self):
        ...
        self.RPI5explorer.generate_for_hw_platform(model= model , model_name = self.model_name)
        assert os.path.exists("experiments/only_for_integration_tests2/models/" + self.model_name) == True
        assert type(torch.jit.load("experiments/only_for_integration_tests2/models/" + self.model_name)) == torch.jit._script.RecursiveScriptModule
        self.tearDown()
    ...
    def tearDown(self):
        ...
        if os.path.exists("experiments/only_for_integration_tests2/models/" + self.model_name):
            os.remove("experiments/only_for_integration_tests2/models/" + self.model_name)
