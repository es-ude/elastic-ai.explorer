...
class TestDeploymentAndMeasurement:
    def setUp(self):
        ...
        self.connection_cfg = ConnectionConfig(config_path="tests/integration/configs/connection_config.yaml")
        self.model_cfg = ModelConfig(config_path="tests/integration/configs/model_config.yaml")
    ...
    def test_run_accuracy_measurement(self):
        self.setUp()
        assert type(self.RPI5explorer.run_accuracy_measurement(model_name= self.model_name, path_to_data="docker/data")) == float
        self.tearDown()

    def test_run_latency_measurement(self):
        self.setUp()
        assert type(self.RPI5explorer.run_latency_measurement(model_name= self.model_name)) == int
        self.tearDown()
