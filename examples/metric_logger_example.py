...
from elasticai.explorer.krepo.metric_logger import KrepoMetricLogger
...
def search_generate_measure_for_pi(
    ...
    explorer.set_metric_logger(KrepoMetricLogger(
        server_ip="<mlflow tracking server_ip>",
        server_port="<mlflow tracking server_port: int>",
        # Optional:
        experiment_name="Raspberry Pi Example Experiment", # Create new experiment or use existing experiment with this name
        nas_config=hw_nas_parameters.to_dict,
        search_space_config=explorer.search_space_cfg,
        hw_platform="Raspberry Pi",
        run_name=None, # Run name to create a new run
        run_id=None, # Run ID to continue an existing run
    ))
    ...
    top_models, top_models_ids = explorer.search(
    ...
    df = measure_on_device(
        explorer, top_models, metric_to_source, retrain_epochs, device, dataset_spec, top_models_ids=top_models_ids
    )
