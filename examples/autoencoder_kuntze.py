...
def get_autoencoder_model(
    ...
    model = Sequential()
    model.append(LayerNorm(normalized_shape=[window_size, num_features])) # input normalization

    # ** encoder
    model.append(LSTM(input_size=num_features, hidden_size=15, num_layers=n_lstm_layers-1, bidirectional=True))
    model.append(LSTM(input_size=15, hidden_size=15, num_layers=1, bidirectional=True))
    model.append(LambdaLayer(lambda x: torch.Tensor.reshape(x, x[:, -1, :])))   # "return_sequences=False" in Keras
    ...
    # ** decoder
    model.append(LSTM(input_size=window_size, hidden_size=15, num_layers=n_lstm_layers, bidirectional=True))
    model.append(
        TimeDistributed(Linear(in_features=15, out_features=num_features))
    )
    model.append(Tanh())
