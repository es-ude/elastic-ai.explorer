...
from search_space.layer_builder import LAYER_REGISTRY
from search_space.registry import ADAPTER_REGISTRY
...
class RepeatType(Enum):
    ...
    def is_depth_needed(self)-> bool:
        if self == RepeatType.REPEAT_OP or RepeatType.REPEAT_PARAMS or RepeatType.VARY_ALL:
            return True
        return False
...
class Sampler:
    ...
    def get_factory(self, repeat_type, block_identifier, block_params):
        if repeat_type == RepeatType.REPEAT_OP:
            return RepeatOpVaryParamsFactory(self.trial, block_identifier, block_params, self.default_op_params)
        elif repeat_type == RepeatType.REPEAT_PARAMS:
            return RepeatOpRepeatParamsFactory(self.trial, block_identifier, block_params, self.default_op_params)
        elif repeat_type == RepeatType.VARY_ALL:
            return VaryAllFactory(self.trial, block_identifier, block_params, self.default_op_params)
        else:
    ...
    def construct_sample(self, search_space: dict):
        ...
        for block in search_space["sequence"]:
            ...
            if repeat_type==RepeatType.REPEAT_BLOCK:
                ...
                model[block_identifier]= model[f"{repeat["ref_block"]}"]
                continue

            if repeat_type== RepeatType.MIRROR_BLOCK:
                continue

            layer_factory = self.get_factory(repeat_type, block_identifier, block_params=block)
...
def construct_model(sample: OrderedDict, in_dim, out_dim):
    ...
    for i, block in sample.items():
        ...
        for layer_index , layer_params in block.items():
            ...
            layers, next_in_shape=insert_needed_adapters(next_in_shape, layer_params["operation"], prev_op, layers)
            layer= LAYER_REGISTRY[layer_params["operation"]]()
            if is_last_layer(i, layer_index, sample):
                build_layer, next_in_shape = layer.build(input_shape=next_in_shape,
                                                         search_parameters=layer_params["params"], output_shape=out_dim)
            else:
                build_layer, next_in_shape= layer.build(input_shape=next_in_shape, search_parameters=layer_params["params"])
...
class Block:
    def __init__(self, trial, block_identifier:str, depth: dict, block_factory):
        print(depth)
        self.depth=parse_search_param(trial, f"depth_b{block_identifier}", depth, "depth", default_value=1)
    ...
    def sample(self):
        for layer in range(self.depth):
            ...
            self.all_searched_params[f"{self.block_identifier}_{layer}"]={"operation": operation, "params": parameters}
...
class RepeatOp(OpCandidates):
    def __init__(self, trial, block_identifier, op_candidates: dict):
        ...
        self.op = parse_search_param(
            trial, f"operation_b{block_identifier}", op_candidates, "op_candidates", default_value=None
        )
    def sample(self):
        return self.op
...
class VaryOp(OpCandidates):
    ...
    def sample(self):
        op= parse_search_param(self.trial, f"operation_b{self.block_identifier}_l{self.layer_num}", self.op_candidates, "op_candidates", default_value=None)
        self.layer_num+=1
        return op

class ParamCandidates(ABC):
    def __init__(self, trial, block_identifier, search_parameters: dict, default_op_params: dict):
        self.trial = trial
        self.block_identifier = block_identifier
    ...
    def sample(self, op: str):
        ...
        if op not in  self.op_search_parameters:
            ...
            for param in op_parameters:
                sampled_value=parse_search_param(self.trial, f"{param}_b{self.block_identifier}", op_parameters, param, default_value=None)
...
class VaryParams(ParamCandidates):
    ...
    def sample(self, op: str):
        ...
        for param in op_parameters:
            ...
            sampled_value=parse_search_param(self.trial, name, op_parameters, param, default_value=None)
            op_dict[param]=sampled_value
        self.layer_num += 1
        return op_dict
...
class RepeatOpVaryParamsFactory(BlockFactory):
    def __init__(self, trial, block_identifier, block_parameters: dict, default_op_params: dict):
        super().__init__(RepeatOp(trial, block_identifier, block_parameters), VaryParams(trial, block_identifier, block_parameters, default_op_params))

class RepeatOpRepeatParamsFactory(BlockFactory):
    def __init__(self,trial, block_identifier, block_parameters: dict, default_op_params: dict):
        super().__init__(RepeatOp(trial, block_identifier, block_parameters), RepeatParams(trial, block_identifier, block_parameters, default_op_params))

class VaryAllFactory(BlockFactory):
    def __init__(self,trial, block_identifier, block_parameters: dict, default_op_params: dict):
        super().__init__(VaryOp(trial, block_identifier, block_parameters), VaryParams(trial, block_identifier, block_parameters, default_op_params))
...
if __name__ == "__main__":
    ...
    study=optuna.create_study(direction="maximize", sampler=RandomSampler(), study_name="study_name5",
                        load_if_exists=True)
    study.optimize(
