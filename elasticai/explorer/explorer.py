...
class Explorer:
    ...
    def hw_setup_on_target(self, connection_conf: DeploymentConfig):
        ...
        self.connection_cfg.dump_as_yaml(
            self._experiment_dir / "connection_config.yaml"
        )

    def run_latency_measurement(self, model_name: str) -> int:
        model_path = self._model_dir / model_name
    ...
    def run_accuracy_measurement(self, model_name: str, path_to_data: str) -> float:
        model_path = self._model_dir / model_name
        self.hw_manager.deploy_model(self.connection_cfg, model_path)
