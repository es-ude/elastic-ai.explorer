...
class FlopsEstimator():
    def __init__(self, model_space: ModelSpace):
        self.logger = logging.getLogger("explorer.nas.costestimator")
    ...
    def estimate_flops_grid(self):
        ...
        self.logger.info("General Flops, Formular: ", profiler.expression)
        for model_sample in self.model_space.grid():
            profiler = FlopsProfiler(model_sample, data_sample)
            self.logger.info("Flops for", model_sample, ": ", profiler.expression)
