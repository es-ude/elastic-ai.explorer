...
def build_search_space_measurements_file(
    latencies: list[int], accuracies_on_device: list[float], accuracies_after_retrain: list[float], metrics_path: Path, model_parameter_path: Path, csv_path: Path
) -> pandas.DataFrame:
    ...
    data_merged["Accuracy after Retrain"] = accuracies_after_retrain
    data_merged["latency in us"] = latencies
    data_merged["Accuracy on Device"] = accuracies_on_device
