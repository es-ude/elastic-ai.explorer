...
class HWManager(ABC):
    ...
    @abstractmethod
    def measure_latency(self, connection_conf: DeploymentConfig, path_to_model) -> int:
        pass

    @abstractmethod
    def install_latency_measurement_on_target(
        self, connection_conf: DeploymentConfig, path_to_program=None
    ):
        pass

    @abstractmethod
    def install_accuracy_measurement_on_target(
        self, connection_conf: DeploymentConfig, path_to_program: str = None
    ):
        pass

    @abstractmethod
    def measure_accuracy(
        self, connection_conf: DeploymentConfig, path_to_model: str
    ) -> float:
        pass
...
class PIHWManager(HWManager):
    ...
    def compile_code(self):
        ...
        self.logger.info(
            "Compilation finished. Programs available in %s", CONTEXT_PATH / "bin"
        )
    ...
    def install_latency_measurement_on_target(
        ...
        with Connection(
            host=connection_conf.target_name, user=connection_conf.target_user
        ) as conn:
            self.logger.info(
                "Install program on target. Hostname: %s - User: %s",
                connection_conf.target_name,
    ...
    def install_accuracy_measurement_on_target(
        ...
        with Connection(
            host=deploy_conf.target_name, user=deploy_conf.target_user
        ) as conn:
            self.logger.info(
                "Install accuracy measurement on target. Hostname: %s - User: %s",
                deploy_conf.target_name,
                deploy_conf.target_user,
            )
            conn.put(path_to_program)
    ...
    def measure_accuracy(
        self, connection_conf: DeploymentConfig, path_to_model: str, path_to_data: str
    ) -> float:
    ...
    def deploy_model(self, connection_conf: DeploymentConfig, path_to_model: str):
        with Connection(
            host=connection_conf.target_name, user=connection_conf.target_user
        ) as conn:
